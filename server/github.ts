...
export class GitHubService {
  ...
  async listRepos(
    ...
    if (cached) {
      ...
      return cached.filter(
          (r) => !excludedRepos.has(r.name.toLowerCase()),
      );
    }
  ...
  private async _fetchRepos(
    ...
    try {
      ...
      console.log(`[cache] repos miss: ${cacheKey} → fetched ${filtered.length} (excluded ${mapped.length - filtered.length})`);
      return filtered;
    } catch (error) {
  ...
  async listCommits(
    ...
    if (options?.cacheOnly) {
      ...
      console.log(
        `[cache] commits cacheOnly: ${owner}/${repo} (${cached.length} commits)`,
      );
      return cached;
    }
    ...
    const existing = this.inflight.get(inflightKey);
    if (existing) {
      console.log(`[cache] commits inflight: ${owner}/${repo} — waiting for existing request`);
  ...
  private async _fetchCommits(
    ...
    console.log(
      `[cache] commits partial: ${owner}/${repo} — ${gaps.length} gap(s) to fetch`,
    );
  ...
  async listContributors(
    ...
    const existing = this.inflight.get(inflightKey);
    if (existing) {
      console.log(`[cache] contributors inflight: ${cacheKey} — waiting for existing request`);
  ...
  async getOverviewStats(
        ...
        batch.map(async (repo) => {
          try {
            ...
            const [commits, prs] = await Promise.all([
              this.listCommits(repo.owner.login, repo.name, { since, until, cacheOnly }),
              fetchPRs
    ...
    const { longestStreak, currentStreak, avgCommitsPerDay } =
      calculateStreaks(allCommits);
