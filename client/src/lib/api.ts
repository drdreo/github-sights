...
export const api = {
  getConfig: () => fetchApi<{ configured: boolean; owner?: string; ownerType?: 'user' | 'org' }>('/config'),
  
  setConfig: (config: ApiConfig) => fetchApi<void>('/config', {
    method: 'POST',
  ...
  getCommits: (owner: string, repo: string, since?: string, until?: string, cacheOnly?: boolean) => {
  ...
  getContributors: (owner: string, repo: string) => fetchApi<Contributor[]>(`/repos/${owner}/${repo}/contributors`),
  ...
  sync: (since?: string, until?: string) => {
    ...
    if (until) params.append('until', until);
    const qs = params.toString();
    return fetchApi<{ synced: number; repos: string[]; errors: string[] }>(`/sync${qs ? `?${qs}` : ''}`, {
      method: 'POST',
    });
  },
};
