...
const REPO_COLORS = [
  ...
  'bg-fuchsia-500', 'bg-lime-500', 'bg-indigo-500'
];
...
export function CommitTimeline({ timelines, startDate, endDate, loading }: CommitTimelineProps) {
  ...
  return (
    <Tooltip.Provider delayDuration={100}>
      ...
      >
          <div className="inline-block relative min-w-full" style={{ width: LABEL_WIDTH + totalWidth }}>
      ...
      >
        <div className="flex items-center gap-2 mb-1">
          <div className={`w-2.5 h-2.5 rounded-md ${repoColor} shadow-sm`} />
           <h3 className="text-sm font-semibold text-gray-200 truncate" title={timeline.repo.full_name}>
            {timeline.repo.name}
          </h3>
        </div>
      ...
      <div className="relative flex">
        ...
        <div className="absolute inset-0 flex">
            {allDays.map((day, i) => {
               ...
               const dayData = timeline.daily.find(d => isSameDay(new Date(d.date), day));
            ...
            <div className="flex items-center justify-between text-gray-400 gap-4">
              <div className="flex items-center gap-2">
                {commit.author.avatar_url ? (
                  <img src={commit.author.avatar_url} alt="" className="w-6 h-6 rounded-full border border-gray-700" />
                ) : (
                ...
                <div className="flex flex-col leading-none gap-0.5">
                   <span className="text-gray-300 font-medium">{commit.author.name}</span>
                   <span className="text-[10px] text-gray-500">{format(new Date(commit.author.date), 'MMM d, yyyy HH:mm')}</span>
                </div>
              </div>
            </div>
      ...
      {[1, 2, 3].map((i) => (
         <div key={i} className="h-[120px] border-b border-dashed border-gray-800 flex">
          ...
          <div className="flex-1 relative flex">
             {[1,2,3,4,5,6].map(col => (
                  <div key={col} className="flex-1 border-r border-gray-800 p-2 flex flex-col gap-2 justify-center">
                      {Math.random() > 0.6 && (
