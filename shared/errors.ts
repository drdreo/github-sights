// ── Error Codes & Factories (shared between API + Crawler) ──────────────────
//
// Pure error types — no framework dependencies (no hono, no Deno-specific).
// The API server wraps these with errorResponse() for HTTP responses.
